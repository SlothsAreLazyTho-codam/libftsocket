...
inline void print_socket_information(TcpServer &server)
{
	...
	std::cout << "\t" << BOLD_MAGENTA << "▲ "
			  << "FTSocket " << FTSOCK_VERSION << RESET << std::endl;
	std::cout << "\t" << BOLD_MAGENTA << "↳ " << RESET
			  << "Local: " << server.getHost() << ":" << server.getPort()
			  << std::endl;
	std::cout << "\t" << BOLD_MAGENTA << "↳ " << RESET << "Debug Mode: enabled"
			  << std::endl;
	std::cout << std::endl;
}
...
int TcpServer::openup(const char *host, const char *port)
{
	...
	if (setsockopt(this->fd.fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) <
		0)
		return (-1);

	if (bind(this->getSocket(), this->getAddress()->ai_addr,
			 this->getAddress()->ai_addrlen) < 0)
		return (-1);
...
/**
 * @brief Enters the main loop of the TCP server, continuously accepting
 * connections and handling events.
 *
 * This function starts the main loop of the TCP server. It continuously checks
 * for events on the registered file descriptors using the `poll` function. It
 * handles various events such as client disconnections, errors, incoming data,
 * and outgoing data. The loop continues until the server is no longer running.
 *
...
void TcpServer::onMessage(TcpClient *a, const std::string &b)
{
	(void)a;
	(void)b;
}
